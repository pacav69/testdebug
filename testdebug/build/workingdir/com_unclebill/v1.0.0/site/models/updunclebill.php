 ...
 * UpdUncleBill Model
 */
class UncleBillModelUpdUncleBill extends JModelForm {
	/**
	 *
	 * @var object item
	 */
	...
	public function getForm($data = array(), $loadData = true) {
		...
		// Get the form.
		$form = $this->loadForm ( 'com_unclebill.updunclebill', 'updunclebill', array (
				'control' => 'jform',
				'load_data' => true 
		) );
		if (empty ( $form )) {
	...
	function &getItem() {
		if (! isset ( $this->_item )) {
			$cache = JFactory::getCache ( 'com_unclebill', '' );
			$id = $this->getState ( 'unclebill.id' );
