<?php
/**
 * @version		$Id: teamslist.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague
...
class dartsleagueModelplayerteamslist extends JModel
{
	...
	function getData() {
        ...
        $query .= " LEFT JOIN #__dartsleague_teams AS t ON t.id = p.catid";
        $query .= " WHERE p.id>0";
		if (!$filter_trash){
			$query .= " AND p.trash=0";
		}
		...
		if ($search) {
			$query .= " AND LOWER( p.name ) LIKE ".$db->Quote('%'.$db->getEscaped($search, true).'%', false);
		}

		if ($filter_state > -1) {
			$query .= " AND p.published={$filter_state}";
		}
		...
		$teams = @array_slice($teams, $pageNav->limitstart, $pageNav->limit);
		return $teams;
	}
	...
	function indentRows( & $rows) {
		...
		$teams = JHTML::_('menu.treerecurse', 0, '', array (), $children);
		return $teams;
	}
	...
	function publish() {
		...
		$row = & JTable::getInstance('dartsleagueteams', 'Table');
		foreach ($cid as $id) {
			$row->load($id);
			$row->publish($id, 1);
		}
	...
	function trash() {
		...
		require_once(JPATH_SITE.DS.'components'.DS.'com_dartsleague'.DS.'models'.DS.'playerteamslist.php');
		$model = new dartsleagueModelplayerteamslist;
		$players = $cid;
		foreach ($cid as $id) {
			$players = @array_merge($teams, dartsleagueModelplayerteamslist::getCategoryChilds($id));
		}
	...
	function restore() {
		...
		$cache->clean();
		if($warning)
			$mainframe->enqueueMessage(JText::_('Some of the players have not been restored because their parent teams is in trash.'), 'notice');
	...
	function remove() {
		...
		if ($warningItems){
			$mainframe->enqueueMessage(JText::_('Some of the players have not been deleted because they have teams.'), 'notice');
		}
		if ($warningChilds){
			$mainframe->enqueueMessage(JText::_('Some of the players have not been deleted because they have child teams.'), 'notice');
		}

		$mainframe->redirect('index.php?option=com_dartsleague&view=players', JText::_('Delete Completed'));
	}

	function playerteamsTree(  $row = NULL, $hideTrashed=false, $hideUnpublished=true) {
        ...
        $query = "SELECT id as value, name as text FROM #__dartsleague_teams WHERE id > 0 {$idCheck}";
...
/*		$children = array ();
		...
		foreach ($list as $player) {
			if($players->trash) $player->treename.=' [**'.JText::_('Trashed players').'**]';
			if(!$players->published) $player->treename.=' [**'.JText::_('Unpublished teams').'**]';
