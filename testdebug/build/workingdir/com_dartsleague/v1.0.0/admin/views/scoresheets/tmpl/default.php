    ...
    <tbody>
      ...
      $k = 0; $i = 0;	$n = count( $this->rows );
			...
			foreach ($this->rows as $row) :
				...
				$link = JRoute::_('index.php?option=com_dl&view=scoresheet&cid='.$row->id);
			?>
      <tr class="<?php echo "row$k"; ?>">
				...
				<td class="dlCenter">
					<?php if(!$this->filter_trash):?>
					<a href="javascript:void(0);" onclick="return listscoresheetTask('cb<?php echo $i;?>','featured')" title="<?php echo ( $row->featured ) ? JText::_( 'Remove featured flag' ) : JText::_( 'Flag as featured' );?>">
					<?php endif; ?>
				...
				<td class="dlCenter"><?php echo ($this->filter_trash) ? strip_tags($published,'<img>') : $published; ?></td>
				<td class="dlOrder">
					<?php if ($this->filter_featured=='1'):?>
					...
					<input type="text" name="order[]" size="5" value="<?php echo $row->featured_ordering; ?>" <?php echo $disabled ?>	class="text_area dlOrderBox" />
					<?php else: ?>
					...
					<?php $disabled = $ordering ?  '' : 'disabled="disabled"'; ?>
					<input type="text" name="order[]" size="5" value="<?php echo $row->ordering; ?>" <?php echo $disabled ?>	class="text_area dlOrderBox" />
					<?php endif; ?>
				</td>
				<td><a href="<?php echo JRoute::_('index.php?option=com_dl&view=category&cid='.$row->catid);?>"><?php echo $row->category; ?></a></td>
				<td>
					<?php if($user->gid>23):?>
				...
				<td class="dlCenter"><?php echo ($this->filter_trash)?strip_tags($access):$access;?></td>
				<td class="dlDate"><?php echo JHTML::_('date', $row->created , JText::_('DL_DATE_FORMAT')); ?></td>
				<td class="dlDate"><?php echo ($row->modified == $nullDate) ? JText::_('Never') : JHTML::_('date', $row->modified , JText::_('DL_DATE_FORMAT')); ?></td>
				<td><?php echo $row->hits ?></td>
				<td class="dlCenter">
					<?php if (JFile::exists(JPATH_SITE.DS.'media'.DS.'dl'.DS.'scoresheets'.DS.'cache'.DS.md5("Image".$row->id).'_XL.jpg')): ?>
					<a href="<?php echo JURI::root().'media/dl/scoresheets/cache/'.md5("Image".$row->id).'_XL.jpg';?>" class="modal"><img src="templates/khepri/images/menu/icon-16-media.png" alt="<?php echo JText::_('Preview image');?>" /></a>
					<?php endif; ?>
				</td>
