  ...
  $k = 0; $i = 0;	$n = count( $this->rows );
	foreach ($this->rows as $row) :
      ...
      <tr class="<?php echo "row$k"; ?>">
        <td><?php echo $i+1; ?></td>
        <td><a class="dlListItemDisabled" title="<?php echo JText::_('Click to add this item'); ?>" onclick="window.parent.jSelectCategory('<?php echo $row->id; ?>', '<?php echo str_replace(array("'", "\""), array("\\'", ""),$row->name); ?>', 'id');"><?php echo $row->treename; ?></a></td>
        <td class="dlCenter"><?php echo $row->extra_fields_group; ?></td>
        <td class="dlCenter"><?php echo strip_tags($access);?></td>
        <td class="dlCenter"><?php echo strip_tags($published,'<img>');?></td>
