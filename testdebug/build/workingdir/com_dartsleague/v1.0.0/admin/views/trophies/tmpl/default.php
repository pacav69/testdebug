    ...
    <tbody>
	  ...
	  $k = 0; $i = 0;	$n = count( $this->rows );
		foreach ($this->rows as $row) :
			...
			$link = JRoute::_('index.php?option=com_dl&view=category&cid='.$row->id);
			?>
        ...
        <td class="dlOrder"><span><?php echo $this->page->orderUpIcon( $i, $row->parent == 0 || $row->parent == @$this->rows[$i-1]->parent, 'orderup', 'Move Up', $ordering); ?></span> <span><?php echo $this->page->orderDownIcon( $i, $n, $row->parent == 0 || $row->parent == @$this->rows[$i+1]->parent, 'orderdown', 'Move Down', $ordering ); ?></span>
          <?php $disabled = $ordering ?  '' : 'disabled="disabled"'; ?>
          <input type="text" name="order[]" size="5" value="<?php echo $row->ordering; ?>" <?php echo $disabled ?> class="text_area dlOrderBox" /></td>
        <td class="dlCenter"><?php echo $row->extra_fields_group; ?></td>
        ...
        <td class="dlCenter">
        <?php if($row->image):?>
        	<a href="<?php echo JURI::root().'media/dl/categories/'.$row->image;?>" class="modal"><img src="templates/khepri/images/menu/icon-16-media.png" alt="<?php echo JText::_('Preview image');?>" /></a>
        <?php endif; ?>
        </td>
