...
<script type="text/javascript">
	...
	function submitbutton(pressbutton) {
		...
		if (trim( document.adminForm.name.value ) == "") {
			alert( '<?php echo JText::_('Season must have a Name', true); ?>' );
		} 
	...
	function addAttachment(){
		var div = new Element('div',{'style':'border-top: 1px dotted #ccc; margin: 4px; padding: 10px;'}).injectInside($('seasonAttachments'));
	...
	window.addEvent('domready', function(){
		...
		$('newTagButton').addEvent('click', function(){
			...
			var url = 'index.php?option=com_dl&view=season&task=tag&tag='+tag;
			new Ajax(url, {
				method: 'get',
		...
		$('catid').addEvent('change', function(){
			...
			var url = 'index.php?option=com_dl&view=season&task=extraFields&cid='+selectedValue;
			<?php if ($this->row->id): ?>
		...
		$('browseSrv').addEvent('click', function(e){
			...
			SqueezeBox.fromElement(this, {
				handler: 'iframe',
				url: '<?php echo JURI::base()?>index.php?option=com_dl&view=season&task=filebrowser&type=image&tmpl=component',
				size: {x: 590, y: 400}
			});
		})
		...
		var completer = new Autocompleter.Ajax.Json($('search-field'), '<?php echo JURI::root()?>index.php?option=com_dl&view=season&task=tags',
			{
		    'postVar': 'q',
      ...
      <tr>
        <td>
 				<table class="adminform adminFormdl">
						...
						<tr>
							<td><label for="description"><?php echo JText::_('Description'); ?></label></td>
							<td><input class="text_area dlTitleBox" type="text" name="description" id="description" maxlength="250" value="<?php echo $this->row->description; ?>" /></td>
						</tr>
                        <tr>
                            <td><label for="year"><?php echo JText::_('Year'); ?></label></td>
                            <td><input class="text_area dlTitleBox" type="text" name="year" id="year" maxlength="250" value="<?php echo $this->row->year; ?>" /></td>
                        </tr>
                        <tr>
                            <td><label for="no_of_weeks"><?php echo JText::_('No of weeks'); ?></label></td>
                            <td><input class="text_area dlTitleBox" type="text" name="no_of_weeks" id="no_of_weeks" maxlength="250" value="<?php echo $this->row->no_of_weeks; ?>" /></td>
                        </tr>
                        <tr>
                            <td><label for="startdate"><?php echo JText::_('Start date'); ?></label></td>
                            <td class="dlItemFormDateField"><?php echo JHTML::_( 'calendar',$this->row->startdate, 'startdate', 'startdate', '%Y-%m-%d'); ?></td>
                         </tr>
                        <tr>
                            <td><label for="enddate"><?php echo JText::_('End date'); ?></label></td>
                            <td class="dlItemFormDateField"><?php echo JHTML::_( 'calendar',$this->row->enddate, 'enddate', 'enddate', '%Y-%m-%d'); ?></td>
                      </table>
        ...
        <td id="adminFormdlSidebar"<?php if(!$this->params->get('sideBarDisplay')): ?> style="display:none;"<?php endif; ?>>
            ...
            <?php if($this->row->id): ?>
            <tr>
              <td><strong><?php echo JText::_('season ID'); ?></strong></td>
...
<!--            <?php if($this->row->id): ?>
          ...
          <table class="admintable">
            ...
            <tr>
              ...
              <td class="dlseasonFormDateField"><?php echo JHTML::_( 'calendar',$this->row->created, 'created', 'created', '%Y-%m-%d %H:%M:%S'); ?></td>
            </tr>
            <tr>
              <td align="right" class="key"><?php echo JText::_('Start publishing'); ?></td>
              <td class="dlseasonFormDateField"><?php echo JHTML::_( 'calendar',$this->row->publish_up, 'publish_up', 'publish_up', '%Y-%m-%d %H:%M:%S'); ?></td>
            </tr>
            <tr>
              <td align="right" class="key"><?php echo JText::_('Finish publishing'); ?></td>
              <td class="dlseasonFormDateField"><?php echo JHTML::_( 'calendar',$this->row->publish_down, 'publish_down', 'publish_down', '%Y-%m-%d %H:%M:%S'); ?></td>
            </tr>
          </table>
...
<!--          <?php echo $pane->startPanel(JText::_('Metadata Information'), "metadata-page"); ?>
          ...
          <?php	echo $pane->endPanel(); ?>  -->
<!--          <?php echo $pane->startPanel(JText::_('season view options in category listings'), "season-view-options-listings"); ?>
          <?php echo $this->form->render('params','season-view-options-listings'); ?>
          <?php echo $pane->endPanel(); ?>
          <?php echo $pane->startPanel(JText::_('season view options'), "season-view-options"); ?>
          <?php echo $this->form->render('params','season-view-options'); ?>
          <?php echo $pane->endPanel(); ?>
