...
class dartsleagueViewvenues extends JView
{
	function display($tpl = null) {
		...
		$filter_venue = $mainframe->getUserStateFromRequest($option.$view.'filter_venue', 'filter_venue',0, 'int');
...
//		$lists['featured'] = JHTML::_('select.genericlist', $filter_featured_options, 'filter_featured', 'onchange="this.form.submit();"', 'value', 'text', $filter_featured);
		...
		JToolBarHelper::title( JText::_('Venues'), 'dartsleague.png' );
		if ($filter_trash == 1) {
			JToolBarHelper::custom('restore','restore.png','restore_f2.png','Restore', true);
			JToolBarHelper::deleteList('Are you sure you want to delete selected venue?', 'remove', 'Delete');
		}
		else{
		...
		JToolBarHelper::preferences('com_dartsleague', '500', '600');
                // new toolbar
        JToolBarHelper::help( 'screen.venues',true ); 
		...
		JSubMenuHelper::addEntry(JText::_('Scoresheets'), 'index.php?option=com_dartsleague&view=scoresheets');
                                // new menu 
        JSubMenuHelper::addEntry(JText::_('Venues'), 'index.php?option=com_dartsleague&view=venues', true);
                                                        // new menu 
        JSubMenuHelper::addEntry(JText::_('Players'), 'index.php?option=com_dartsleague&view=players');
                                                                        // new menu 
	...
	function move(){
		...
		foreach ($cid as $id) {
			$row = & JTable::getInstance('dartsleagueItem', 'Table');
			$row->load($id);
			$rows[]=$row;
		}
		...
		JToolBarHelper::title( JText::_('Move venue'), 'dartsleague.png' );
