...
class JElementteam extends JElement
{
	...
	{
		$db = &JFactory::getDBO();
		$query = 'SELECT m.*, v.name AS venuename FROM #__dartsleague_teams m WHERE published = 1 ORDER BY ordering';
        ...
        // $query .= " LEFT JOIN #__dartsleague_venues AS v ON v.id = i.catid";
        //$query = 'SELECT m.* FROM #__dartsleague_teams m WHERE published = 1 ORDER BY parent, ordering';
		$db->setQuery( $query );
		...
		if($name=='teams'){
			...
			function setTask() {
				var counter=0;
				$$('#paramsteams option').each(function(el) {
					if (el.selected){
						value=el.value;
						counter++;
					}
				...
				if (counter==1){
					...
					$('urlparamstask').setProperty('value','team');
					disableParams();
				}
			}
