<?php
/**
 * @version		$Id: Venue.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague
...
class JElementVenue extends JElement
{
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		if($name=='venues'){
			...
			function setTask() {
				var counter=0;
				$$('#paramsvenues option').each(function(el) {
					if (el.selected){
						value=el.value;
						counter++;
					}
				...
				if (counter==1){
					...
					$('urlparamstask').setProperty('value','venue');
					disableParams();
				}
			}
		...
		foreach ( $list as $item ) {
			@$vitems[] = JHTML::_('select.option',  $item->id, '&nbsp;&nbsp;&nbsp;'. $item->treename.$item->title );
		}

		return JHTML::_('select.genericlist',  $vitems, ''.$control_name.'['.$name.'][]', 'onchange="setTask();" class="inputbox" style="width:90%;" multiple="multiple" size="15"', 'value', 'text', $value );
