<?php
/**
 * @version		$Id: dartsleaguescoresheet.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague
...
class TabledartsleagueScoresheet extends JTable {
  ...
  var $email = null;
    ...
    */
	function __construct(&$db) {
		...
		parent::__construct('#__dartsleague_scoresheet', 'id', $db);
	}

	function check() {

		if (trim($this->title) == '') {
			$this->setError(JText::_('scoresheet must have a title'));
			return false;
		}
		if ($this->catid == '0') {
			$this->setError(JText::_('scoresheet must have a category'));
			return false;
		}
		if ( empty($this->alias)) {
	...
	function getNextOrder($where = '', $column = 'ordering') {
		...
		$query = "SELECT MAX({$column}) FROM #__dartsleague_scoresheet";
	...
	function reorder($where = '', $column = 'ordering') {
		...
		$k = $this->_tbl_key;
		$query = "SELECT {$this->_tbl_key}, {$column} FROM #__dartsleague_scoresheet WHERE {$column}>0";
		...
		for ($i = 0, $n = count($orders); $i < $n; $i++) {
			if ($orders[$i]->$column >= 0) {
				if ($orders[$i]->$column != $i + 1) {
					$orders[$i]->$column = $i + 1;
					$query = "UPDATE #__dartsleague_scoresheet SET {$column}=".(int) $orders[$i]->$column;
