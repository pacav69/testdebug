<?php
/**
 * @version		$Id: dartsleagueplayer.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague player table
...
class Tabledartsleagueplayer extends JTable {
  ...
  var $email = null;
    ...
    */
	function __construct(&$db) {
		...
		parent::__construct('#__dartsleague_players', 'id', $db);
	}

	function check() {

		if (trim($this->firstname) == '') {
			$this->setError(JText::_('Player must have a First Name'));
			return false;
		}
/*        if ($this->teamid == '0') {
            $this->setError(JText::_('Player must have a team'));
            return false;
        }  */
  ...
  /**
	...
	function getNextOrder($where = '', $column = 'ordering') {
		...
		$query = "SELECT MAX({$column}) FROM #__dartsleague_players";
	...
	function reorder($where = '', $column = 'ordering') {
		...
		$k = $this->_tbl_key;
		$query = "SELECT {$this->_tbl_key}, {$column} FROM #__dartsleague_players WHERE {$column}>0";
		...
		for ($i = 0, $n = count($orders); $i < $n; $i++) {
			if ($orders[$i]->$column >= 0) {
				if ($orders[$i]->$column != $i + 1) {
					$orders[$i]->$column = $i + 1;
					$query = "UPDATE #__dartsleague_players SET {$column}=".(int) $orders[$i]->$column;
