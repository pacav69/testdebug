<?php
/**
 * @version		$Id: dartsleagueseason.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague season table
...
class Tabledartsleagueseason extends JTable {
    var $id = null;
    var $name = null;
    var $description = null;
    var $year = null;
    var $no_of_weeks = null;     
    var $startdate = null;    
    var $enddate = null; 
    var $published = null;
  ...
  var $email = null;
    ...
    */
	function __construct(&$db) {
		...
		parent::__construct('#__dartsleague_seasons', 'id', $db);
	}

	function check() {

		if (trim($this->name) == '') {
			$this->setError(JText::_('Season must have a Name'));
			return false;
		}
  /**
	...
	function getNextOrder($where = '', $column = 'ordering') {
		...
		$query = "SELECT MAX({$column}) FROM #__dartsleague_seasons";
	...
	function reorder($where = '', $column = 'ordering') {
		...
		$k = $this->_tbl_key;
		$query = "SELECT {$this->_tbl_key}, {$column} FROM #__dartsleague_seasons WHERE {$column}>0";
		...
		for ($i = 0, $n = count($orders); $i < $n; $i++) {
			if ($orders[$i]->$column >= 0) {
				if ($orders[$i]->$column != $i + 1) {
					$orders[$i]->$column = $i + 1;
					$query = "UPDATE #__dartsleague_seasons SET {$column}=".(int) $orders[$i]->$column;
