<?php
/**
 * @version		$Id: dartsleagueteam.php 478 2010-06-16 16:11:42Z SilverPC Consultants. $
 * @package		dartsleague team table
...
class TableDLTeam extends JTable {
  ...
  var $email = null;
    ...
    */
	function __construct(&$db) {
		...
		parent::__construct('#__dartsleague_teams', 'id', $db);
	}

	function check() {

		if (trim($this->name) == '') {
			$this->setError(JText::_('Team must have a Name'));
			return false;
		// }
  /**
		...
		if(JPluginHelper::isEnabled('system', 'unicodeslug'))
			...
			$teams = explode(',', $params->get('SEFReplacements'));
			foreach ($teams as $team) {
				if (! empty($team)) {
					@list($src, $dst) = explode('|', trim($team));
	...
	function getNextOrder($where = '', $column = 'ordering') {
		...
		$query = "SELECT MAX({$column}) FROM #__dartsleague_teams";
	...
	function reorder($where = '', $column = 'ordering') {
		...
		$k = $this->_tbl_key;
		$query = "SELECT {$this->_tbl_key}, {$column} FROM #__dartsleague_teams WHERE {$column}>0";
		...
		for ($i = 0, $n = count($orders); $i < $n; $i++) {
			if ($orders[$i]->$column >= 0) {
				if ($orders[$i]->$column != $i + 1) {
					$orders[$i]->$column = $i + 1;
					$query = "UPDATE #__dartsleague_teams SET {$column}=".(int) $orders[$i]->$column;
